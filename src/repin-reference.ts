...
const semaphoreRestamp = new Semaphore(1)
...
function refreshReupload(total?:number) {
	...
	if (true || checkNodes > 0 || checkFiles > 0 || totalFiles > 0) {
		...
		const text = `Check:${checkNodes}+${checkFiles}-${pinnedFiles}/${totalFiles} !Retr:${failedNodes}+${failedFiles} Fails:${nodeFails}+${fileFails} Fatal:${node2Fails}+${file2Fails} Pins:${createdPins}-${failedPins} Pending:Nodes:${pendingNodes.length}(${nodesRunning}-${nodesSleeping})+${pendingValues.length}(${valuesRunning}-${valuesSleeping}) Files:${pendingFiles.length}(${filesRunning}-${filesSleeping}) rss:${Math.floor(heap.rss/1024/1024)}MB`
		showTopLine(text)
...
async function pinReference(reference: string, type: string, path: string, deletePins: boolean) : Promise<boolean> {
...
async function reStamp(reference: string, what: string, chunk: string) {
	waitForHold(`reStamp(what)`)

	const [value, release] = await semaphoreRestamp.acquire();

	showBoth(`reStamping ${reference} ${what} chunk ${chunk}`)
	
	try {
		const content = await executeBinaryAPI(beeUrl, 'chunks', chunk)
		const bytes = content.length
		showLog(`reStamp get chunk ${chunk} got ${bytes}`)
		if (bytes == 0) throw new Error('Zero Bytes Read');
		//var content = await executeAPI(beeUrl, 'bytes', 'get', address)
		
		try {
			
			const delResult = await executeAPI(beeUrl, 'chunks', chunk, 'DELETE')
			showBoth(`reStamp delete ${chunk} got ${JSON.stringify(delResult)}`)
			try {
				
				if (tagID == 0) {
					const startTag = await bee.createTag()
					tagID = startTag.uid
					showBoth(`reStamp using tagID ${tagID}`)
				}
				
				const newRef = (await executeAPI(beeUrl, 'chunks', '', 'POST', {"Content-Type": "application/octet-stream", "swarm-postage-batch-id": batchID, "swarm-tag": `${tagID}`, "swarm-pin": true}, content)).reference
				if (chunk != newRef)
					showBoth(`reStamp chunk ${chunk} DOESN'T MATCH newRef ${newRef}`)
			}
			catch (err2) {
				showBoth(`reStamp ${what} chunk ${chunk} POST failed with ${err2}`)
			}
		}
		catch (err3)
		{
			showBoth(`reStamp ${what} chunk ${chunk} DELETE failed with ${err3}`)
		}
		
	}
	catch (err) {
		showBoth(`reStamp ${what} chunk ${chunk} GET failed with ${err}`)
	}
	finally {
		release()
	}
...
async function isRetrievable(reference : string, what : string = "*unknown*", retries : number = 10) : Promise<Boolean> {
	...
	for (let retry=0; retry<retries; retry++) {
		...
		try {
			...
			start = new Date().getTime()
			status = await executeAPI(beeUrl, 'stewardship', reference, 'track')	// Expand all chunks and get stamps
			//console.log( status )
			elapsed = Math.trunc((new Date().getTime() - start)/100+0.5)/10.0
			//showBoth(`Track ${reference} gave ${status.chunks.length} chunks`)
			var pinIssues = 0
			var pinMin = 0, pinMax = 0
			for (const chunk of status.chunks) {
				if (chunk.pinCount < 9000) {
					if (pinIssues == 0) {
						pinIssues++
						pinMin = chunk.pinCount; pinMax = chunk.pinCount;
					} else {
						pinIssues++
						if (chunk.pinCount < pinMin) pinMin = chunk.pinCount
						if (chunk.pinCount > pinMax) pinMax = chunk.pinCount
					}
				}
			}
			if (pinIssues > 0) {
				showBoth(`${reference} ${what} ${pinIssues}/${status.chunks.length} pinIssues from ${pinMin} to ${pinMax}`)
				const pattern = /(.+)\((.+)\)/
				const matches = what.match(pattern)
				if (matches != null && matches.length == 3) {
					await pinReference(reference, matches[1], matches[2], true)
				} else {
					showBoth(`${reference} failed to parse ${what}`)
				}
			}
			if (batchID != "") {
				for (const chunk of status.chunks) {
					if (chunk.batch != batchID) {	// Only restamp the ones that are not correct
						showBoth(`${reference} ${what} (${status.chunks.length}) includes ${JSON.stringify(chunk)}`)
						//if (chunk.address == "140810907df8ec34aed59d1cc53d8a403b267db69219b2c7c2315c4354b8a846"		//node(images/marker-shadow.png)
						//|| chunk.address == "ba0080c7298e812e2bb0ed047422d0457231aef3ba6acd3d2b8dc874d66286df") {	//file(images/marker-shadow.png)
							await reStamp(reference, what, chunk.address)
						//}
					}
				}
			}
			...
			elapsed = Math.trunc((new Date().getTime() - start)/100+0.5)/10.0
			showBoth(`isRetrievable ${reference} ${what} retry:${retry} failed with ${err} in ${elapsed} seconds`)
			try {
				const content = await downloadData(reference, what)
				//if (what.substring(0,5) == 'node(')
				//await reStamp(reference, what, reference)
				showBoth(`isRetrievable ${reference} ${what} retry:${retry} downloadData got ${content.length}, sleeping...`)
				await sleep(5000)
			}
			catch (err) {
				showBoth(`isRetrievable ${reference} ${what} retry:${retry} downloadData failed with ${err}`)
			}
			//throw err
		}
		finally {
		...
		try {
			start = new Date().getTime()
			if (await reupload(reference, what+'-retr')) {
				elapsed = Math.trunc((new Date().getTime() - start)/100+0.5)/10.0
...
//	if (DeletePins || !await getPin(entry)) {
	if (await isRetrievable(entry, `file(${prefix})`)) {
		try {
			checkFiles++
			const content = await downloadData(entry, prefix)
			showLog(`${indent}${prefix} got ${content.length} bytes`)
			await pinReference(entry, 'file', prefix, DeleteBeforePin)
		} catch (err: any) {
...
async function printAllForks(storageLoader: StorageLoader, node: MantarayNode, reference: Reference|undefined, prefix: string, indent: string, what: string, filter: string|undefined, excludes: string[]|undefined, manifestOnly: Boolean, loadFiles: Boolean, saveFiles: Boolean): Promise<void> {
	...
	try {
		...
		await pinReference(bytesToHex(reference), 'node', prefix, DeleteBeforePin)	// Only pin it after a successful load!
	}
	catch (err: any) {
