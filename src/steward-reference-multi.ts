...
function beeUrl() {
	const result = beeUrls[Math.floor(Math.random()*beeUrls.length)];
//	showBoth(`using ${result}`)
	return result
}
...
const semaphoreRetrievableNode = new Semaphore(RetrievableMax);
const semaphoreRetrievableFile = new Semaphore(RetrievableMax);
...
function refreshReupload(total?:number) {
	...
	if (true || checkNodes > 0 || checkFiles > 0 || totalFiles > 0) {
		...
		const text = `Check:${checkNodes}+${checkFiles}-${pinnedFiles}/${totalFiles} !Retr:${failedNodes}+${failedFiles} Fails:${nodeFails}+${fileFails} Fatal:${node2Fails}+${file2Fails} Pending:Nodes:${pendingNodes.length}(${nodesRunning}-${nodesSleeping})+${pendingValues.length}(${valuesRunning}-${valuesSleeping}) Files:${pendingFiles.length}(${filesRunning}-${filesSleeping}) rss:${Math.floor(heap.rss/1024/1024)}MB`
		showTopLine(text)
...
// Note: The pin functions are short-circuited for /stewardship checking

async function getPin(beeUrl: string, reference: string) : Promise<boolean> {
	return false
	try {
...
async function createPin(beeUrl: string, reference: string) : Promise<boolean> {
	return false
	try {
...
async function deletePin(beeUrl: string, reference: string) : Promise<boolean> {
	return true
        try {
...
async function pinReference(beeUrl: string, reference: string, type: string, path: string) : Promise<boolean> {
	return true
	const [value, release] = await semaphorePin.acquire();
	try {
		if (DeletePins) {
                	printStatus(`Deleting pin ${type} ${path} at ${reference}`)
                	await deletePin(beeUrl, reference)
		} else {
			printStatus(`Checking pin ${type} ${path} at ${reference}`)
			if (await getPin(beeUrl, reference)) return true
		}
		printStatus(`Actually pinning ${type} ${path} at ${reference}`)
		const result = createPin(beeUrl, reference)
		if (await result) {
			createdPins++
...
async function reupload(beeUrl: string, reference : string, what : string = "*unknown*", retries : number = 10) : Promise<Boolean> {
	...
	for (let retry=0; retry<retries; retry++) {
		...
		try {
			...
			status = await executeAPI(beeRecoveryUrl, 'stewardship', reference, 'put', { 'User-Agent': what, 'swarm-postage-batch-id': batchID, 'swarm-steward-with-manifest': 'false' } )
			//console.log( status )
		...
		if (status) {
			try {
				start = new Date().getTime()
				await downloadData(beeUrl, reference, what)
				elapsed = Math.trunc((new Date().getTime() - start)/100+0.5)/10.0
...
async function isRetrievable(beeUrl: string, reference : string, what : string = "*unknown*", retries : number = 10) : Promise<Boolean> {
	const isNode = (what.substring(0,5) == 'node(')
	if (! beeUrl ||  beeUrl == '') {
	...
	//printStatus(`isRetrievable ${reference} ${what} queued`)

	for (let retry=0; retry<retries; retry++) {
		var sem = isNode ? semaphoreRetrievableNode : semaphoreRetrievableFile
		var start = new Date().getTime()
		const [value, release] = await sem.acquire();
		var elapsed = Math.trunc((new Date().getTime() - start)/100+0.5)/10.0
		try {
			//printStatus(`isRetrievable ${reference} ${what} retry:${retry} after ${elapsed} seconds sem acquire`)
			start = new Date().getTime()
			status = await executeAPI(beeUrl, 'stewardship', reference, 'get')
			//console.log( status )
			elapsed = Math.trunc((new Date().getTime() - start)/100+0.5)/10.0
			...
			showBoth(`isRetrievable ${reference} ${what} retry:${retry} status:${JSON.stringify(status)} elapsed ${elapsed} seconds`)
		}
		catch (err) {
			elapsed = Math.trunc((new Date().getTime() - start)/100+0.5)/10.0
		...
		if (isNode) failedNodes++; else failedFiles++
		try {
			start = new Date().getTime()
			if (await reupload(beeUrl, reference, what+'-retr')) {
				elapsed = Math.trunc((new Date().getTime() - start)/100+0.5)/10.0
				showLog(`isRetrievable ${reference} ${what} reupload succeeded in ${elapsed} seconds`)
			} else {
				elapsed = Math.trunc((new Date().getTime() - start)/100+0.5)/10.0
				showBoth(`isRetrievable ${reference} ${what} reupload failed in ${elapsed} seconds`)
			}
		}
		catch (err) {
			elapsed = Math.trunc((new Date().getTime() - start)/100+0.5)/10.0
			showBoth(`isRetrievable ${reference} ${what} reupload failed with ${err} in ${elapsed} seconds`)
		}
	}
	showBoth(`isRetrievable ${reference} ${what} FAILED all retries!`)
	return false;
}
...
async function checkFile(beeUrl: string, entry: string, prefix: string, indent: string) {
//	if (DeletePins || !await getPin(entry)) {
	if (await isRetrievable(beeUrl, entry, `file(${prefix})`)) {
		try {
			checkFiles++
			const content = await downloadData(beeUrl, entry, prefix)
			showLog(`${indent}${prefix} got ${content.length} bytes`)
			await pinReference(beeUrl, entry, 'file', prefix)
		} catch (err: any) {

			if (!await reupload(beeUrl, entry, `file(${prefix})`)) {
			...
			for (let r=1; r<=30; r++) {
				...
				await sleep(r*1000)
				try {
					const content2 = await downloadData(beeUrl, entry, prefix)
...
//		showLog(`${indent}${prefix} pin exists, skipping download!`)
                showLog(`${indent}${prefix} NOT Retrievable!`)
	}
}

async function processNodeOrValue(what: string, manifestOnly: Boolean, loadFiles: Boolean, saveFiles: Boolean) : Promise<Boolean>
{
	...
	if (node && !exitRequested) {
		...
		const timeout = setTimeout(async () => {
						if (!exitRequested && !Holding && nodesRunning < RetrievableMax) {
							nodesRunning++
							while (running)
								if (!await processNodeOrValue(what, manifestOnly, loadFiles, saveFiles))
									break
							nodesRunning--
						}
					}, 6000);	// Longer than 6 seconds, spawn a parallel thread
		await printAllForks(beeUrl(), node.node, node.node.getEntry, node.prefix, node.indent, what, undefined, node.excludes, manifestOnly, loadFiles, saveFiles);
...
async function processNodes(what: string, manifestOnly: Boolean, loadFiles: Boolean, saveFiles: Boolean)
{
	var retries = 5
	...
	while (!exitRequested && ((elapsed < 60 && nodesActive > 0) || retries-- > 0)) {	// Require 60 seconds before allowing termination
		while (pendingNodes.length > 0 || pendingValues.length > 0) {
			await processNodeOrValue(what, manifestOnly, loadFiles, saveFiles)
...
async function processValue(what: string, manifestOnly: Boolean, loadFiles: Boolean, saveFiles: Boolean) : Promise<Boolean>
{
	var node: pendingNode | undefined
	...
	if (node && !exitRequested) {
		...
		const timeout = setTimeout(async () => {
						if (!exitRequested && !Holding && valuesRunning < RetrievableMax) {
							valuesRunning++
							while (running)
								if (!await processValue(what, manifestOnly, loadFiles, saveFiles))
									break
							valuesRunning--
						}
					}, 6000);	// Longer than 6 seconds, spawn a parallel thread
...
async function processValues(what: string, manifestOnly: Boolean, loadFiles: Boolean, saveFiles: Boolean)
{
	const start = new Date().getTime()
	var elapsed = Math.trunc((new Date().getTime() - start)/1000+0.5)
	while (nodesRunning > 0 || pendingValues.length > 0) {
		while (pendingValues.length > 0) {
			await processValue(what, manifestOnly, loadFiles, saveFiles)
...
async function processFile(what: string, loadFiles: Boolean, saveFiles: Boolean) : Promise<Boolean>
{
	if (pendingFiles.length > 0) {
		...
		if (file && !exitRequested) {
			...
			await checkFile(beeUrl(), file.reference, file.prefix, file.indent);
			clearTimeout(timeout)
			running = false
			return true
		}
...
async function printAllForks(beeUrl: string, node: MantarayNode, reference: Reference|undefined, prefix: string, indent: string, what: string, filter: string|undefined, excludes: string[]|undefined, manifestOnly: Boolean, loadFiles: Boolean, saveFiles: Boolean): Promise<void> {

	if (!reference) return
	
	if (exitRequested) return

	const myLoader = async (address: Reference): Promise<Uint8Array> => {
		return downloadData(beeUrl, bytesToHex(address), `myLoader(node(${prefix}))`)
	}
	
	try {
		checkNodes++
		await isRetrievable(beeUrl, bytesToHex(reference), `node(${prefix})`)
		await node.load(myLoader, reference)
		await pinReference(beeUrl, bytesToHex(reference), 'node', prefix)	// Only pin it after a successful load!
	}
	catch (err: any) {
		var badAddr = bytesToHex(reference)

		if (!await reupload(beeUrl, badAddr, `node(${prefix})`)) {
	...
	if (entry != zeroAddress) {
		...
		if (loadFiles && what && what != "") {
			...
			else await checkFile(beeUrl, entry, prefix, indent)
		}
	}
		...
		if (filter && filter != '') {
			...
			if (newPrefix.slice(0,checkLen) == filter.slice(0,checkLen)) {
				if (checkLen < filter.length) {
					...
					await printAllForks(beeUrl, fork.node, fork.node.getEntry, newPrefix, indent+'  ', what, filter, excludes, manifestOnly, loadFiles, saveFiles)
					continue
				}
...
async function dumpManifest(beeUrl: string, reference: string, what: string, filter: string|undefined = undefined, excludes: string[]|undefined, manifestOnly = false, loadFiles: Boolean = true, saveFiles: Boolean = false) : Promise<void> {

	var start = new Date().getTime()
	showLog(`dumpManifest:${what} from ${reference} ${filter}`)

	let node = new MantarayNode()
	const result = printAllForks(beeUrl, node, hexToBytes(reference), '', '', what, filter, excludes, manifestOnly, loadFiles, saveFiles)
	...
	for (let i = 0; i < 20; i++) {
		nodesRunning++
		promiseArray.push(processNodes(what, manifestOnly, loadFiles, saveFiles))
	}

	for (let i = 0; i < 20; i++) {
		valuesRunning++
		promiseArray.push(processValues(what, manifestOnly, loadFiles, saveFiles))
	}
...
async function uploadData(beeUrl: string, content: Uint8Array | string, what: string, pin: boolean) : Promise<Reference> {
...
//	return uploadData(beeUrl, data, `saveFunction(${data.length})`, true)
//	try {
...
async function downloadData(beeUrl: string, address: string, what : string = "*unknown*") : Promise<Uint8Array> {
...
//	return downloadData(beeUrl, bytesToHex(address), "loadFunction")
//	var start = new Date().getTime()
//	var bytes = 0
//	var r
//	try {
